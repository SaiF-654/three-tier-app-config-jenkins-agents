...
""")
        ...
        stage('Production Approval'){
            steps{ input message: "Deploy to PRODUCTION? Tag: ${IMAGE_TAG}", ok: "Deploy" }
        }
    ...
    post {
        success { echo "✅ Production deployed: ${IMAGE_TAG}" }
        failure { echo "❌ Production deployment failed." }
        aborted { echo "⚠️ Production deployment aborted." }
    }
}
